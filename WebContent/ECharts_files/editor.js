...
function initEcharts() {
	gb.chart = echarts.init($("#chart-panel")[0]), gb.editor.setValue("var option = {\n    \n};\n"), gb.editor.selection.setSelectionRange({
		start: {
			row: 1,
			column: 4
		},
...
function updateEchartsVersion() {
	...
	if (!echarts || version !== echarts.version || gb.echartsSource[version]) try {
		...
		log("Loading ECharts Version Failed！", "error")
	}
}
function loadEchartsOfAllVersions() {
	for (var e = ["2.2.7", "3.0.0"], t = 0, n = e.length; n > t; ++t)!
	function(t) {
		$.get("./vendors/echarts/echarts-all-" + e[t] + ".js", function(n) {
			gb.echartsSource[e[t]] = n
		}, "text").fail(function() {
			console.error("Loading ECharts " + e[t] + " Failed！")
		})
	}(t)
}
function save() {
	return IS_STATIC ? void console.warn("Static version can not use this function!") : void $.get("http://" + window.location.host + "/chart/add", {
		...
		log("Can not save to server, cached locally. Try to refresh page", "error"), console.error(e)
	})
}
function fork() {
	if (IS_STATIC) return void console.warn("Static version can not use this function!");
	var e = getCid(),
		t = getVersion();
...
function localSave() {
	...
	if (window.localStorage && e !== gb.loadedCode) try {
		...
		console.error(t), log("Can not save to server, cached locally. Try to refresh page")
	}
}
function localLoad() {
	try {
		var e = window.localStorage.getItem("code");
		e && (gb.editor.setValue(e), log("Load success!"))
	} catch (t) {
		console.error(t)
	}
}
...
function hasEditorError() {
	for (var e = gb.editor.getSession().getAnnotations(), t = 0, n = e.length; n > t; ++t) if ("error" === e[t].type) return !0;
	return !1
}
function load() {
	if (IS_STATIC) return void(configs.c && $.ajax("./data/" + configs.c + ".js", {
		...
		log("Failed loading chart", "error")
	}));
	...
	"new" !== e ? $.get("http://" + window.location.host + "/chart/find/" + e, {
		...
		var t = e.creater || "Anonymous User",
			n = formatTime(new Date(e.createTime));
		log(t +  "created this chart " + "at " + n )
	}).fail(function(e) {
		console.error(e), window.location.pathname = "new"
	}) : (log("Welcome ECharts Playground！"), window.localStorage ? (localLoad(), run()) : log("Browser doesn't support local cache, remember save the chart! ", "warn"))
}
function log(e, t) {
...
var appEnv = {},
	run = function(ignoreOptionNotChange) {
		if (hasEditorError()) return void log("Something wrong in editor's content! ", "error");
		...
		try {
			...
			if (option = null, eval(gb.editor.getValue()), option && "object" == typeof option && (!_.isEqual(option, gb.lastOption) || ignoreOptionNotChange)) {
				...
				log("Chart generated, " + gb.updateTime + "ms")
			}
		} catch (e) {
			log("Something wrong in editor's content!", "error"), console.error(e)
		}
	},
